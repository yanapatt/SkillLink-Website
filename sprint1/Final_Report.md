...
2.  **การจัดการ Bug**
    *   ออกแบบตาราง Test Case เพื่อวางแผนการทดสอบเว็ปไซต์
4.  **ทำเอกสารประกอบโครงงาน**
...
2.  **การจัดการ Bug**
    *   ใช้ Jest จาก Node JS ในการเขียน Test Case เพื่อทดสอบเว็ปไซต์
4.  **ทำเอกสารประกอบโครงงาน**
...
| AccountService | createAccount | should default accRole to "User" if not provided | ทดสอบว่าถ้าไม่ส่ง `accRole` มา จะตั้งค่า `accRole` เป็น `"User"` โดยอัตโนมัติ |
| AccountService | authenticateAccount | should return account if username and password are correct | ทดสอบการล็อกอินสำเร็จ เมื่อชื่อผู้ใช้และรหัสผ่านถูกต้อง โดยเทียบรหัสผ่านหลังเข้ารหัสแล้ว |
| AccountService | authenticateAccount | should return null if username does not exist | ทดสอบกรณีที่ไม่มีชื่อผู้ใช้นี้ในระบบ ต้องคืนค่า `null` |
| AccountService | authenticateAccount | should return null if password is incorrect | ทดสอบกรณีที่รหัสผ่านไม่ถูกต้อง ถึงแม้มีชื่อผู้ใช้นั้นอยู่ ต้องคืนค่า `null` |
| AccountService | authenticateAccount | should return null and log error if account not found | ทดสอบกรณีที่หาไม่เจอข้อมูลบัญชีจากฐานข้อมูล ถึงแม้จะมี size=1 แต่ไม่มี node จริง ต้อง log error |
...
| ImageRepository | uploadImage | should upload an image | ทดสอบการอัปโหลดไฟล์รูปภาพสำเร็จ และส่งต่อไปยัง middleware ถัดไป |
| ImageRepository | uploadImage | should handle invalid file type with wrong mimetype | ทดสอบว่าแม้ mimetype ไม่ถูกต้อง ระบบยังคงยอมรับการอัปโหลดได้ (ตาม logic ที่ไม่ตรวจสอบ mimetype) |
| ImageRepository | uploadImage | should handle missing file in request | ทดสอบกรณีไม่มีไฟล์ใน request แล้วต้องตอบกลับด้วย error 400 |
| ImageRepository | saveImage | should save image URL correctly | ทดสอบการบันทึก URL ของไฟล์ที่อัปโหลดไว้ในโฟลเดอร์ `/uploads/` |
| ImageRepository | saveImage | should throw error when saving image with invalid data | ทดสอบว่าโยนข้อผิดพลาดออกมาเมื่อข้อมูลไฟล์ไม่ถูกต้อง |
| ImageRepository | removeImage | should remove image from filesystem | ทดสอบการลบไฟล์รูปภาพออกจากระบบไฟล์สำเร็จ |
| ImageRepository | removeImage | should handle error when removing image | ทดสอบว่าระบบสามารถจัดการกับข้อผิดพลาดที่เกิดจากการลบไฟล์ได้ (ไฟล์ไม่เจอ) |
| ImageRepository | removeImage | should handle ENOENT error gracefully | ทดสอบว่าไม่แสดงข้อผิดพลาดถ้าไฟล์ไม่เจอ (ENOENT) |
| ImageRepository | removeImage | should handle unexpected errors gracefully | ทดสอบการจัดการกับข้อผิดพลาดอื่น ๆ ที่ไม่ใช่ ENOENT อย่างเหมาะสม |
| ImageRepository | removeImage | should not attempt to remove image if imgUrl is not provided | ทดสอบว่าถ้าไม่มี URL ของรูปภาพ จะไม่พยายามลบไฟล์ใด ๆ |
| ImageRepository | generateFilename | should return a filename with timestamp and original name | ทดสอบการสร้างชื่อไฟล์ใหม่ โดยมี timestamp 13 หลักนำหน้า แล้วตามด้วยชื่อไฟล์ต้นฉบับ |
| ImageRepository | configureStorage | should return diskStorage with destination and filename functions | ทดสอบการตั้งค่า diskStorage ของ multer ว่ามีทั้งฟังก์ชันกำหนดโฟลเดอร์และชื่อไฟล์ |
| ImageRepository | configureStorage | should call destination callback with correct path | ทดสอบว่า callback สำหรับ destination ถูกเรียกด้วย path ที่ถูกต้อง |
| ImageRepository | configureStorage | should call filename callback with generated filename | ทดสอบว่า callback สำหรับ filename ถูกเรียกด้วยชื่อไฟล์ที่สร้างใหม่จาก originalname |
...
| --- | --- | --- | --- |
| **PostService** | **CREATE** | **createPost** | การทดสอบฟังก์ชัน `createPost` ในกรณีที่สร้างโพสต์ที่มีและไม่มีรูปภาพ รวมถึงกรณีที่มีข้อผิดพลาดในการบันทึกรูปภาพหรือโพสต์ |
|  |  | should create post with image | ทดสอบการสร้างโพสต์ที่มีรูปภาพ |
|  |  | should create post without image | ทดสอบการสร้างโพสต์ที่ไม่มีรูปภาพ |
|  |  | should log an error if image saving fails | ทดสอบกรณีที่บันทึกรูปภาพไม่สำเร็จแล้วเกิดข้อผิดพลาด |
|  |  | should log an error if creating post fails | ทดสอบกรณีที่การสร้างโพสต์ไม่สำเร็จแล้วเกิดข้อผิดพลาด |
| **PostService** | **READ** | **calculateAverageRating** | การทดสอบการคำนวณค่าเฉลี่ยการให้คะแนนในโพสต์ต่างๆ |
|  |  | should return 0 for no posts | ทดสอบเมื่อไม่มีโพสต์จะต้องคืนค่าเป็น 0 |
|  |  | should calculate average rating | ทดสอบกรณีที่มีการให้คะแนนในโพสต์และคำนวณค่าเฉลี่ยการให้คะแนน |
|  |  | should return 0 when posts have no ratings | ทดสอบกรณีที่โพสต์ไม่มีการให้คะแนนและต้องคืนค่าเป็น 0 |
| **PostService** | **UPDATE** | **ratingPost** | การทดสอบฟังก์ชัน `ratingPost` ที่ให้คะแนนโพสต์ |
|  |  | should add a new rating | ทดสอบการเพิ่มคะแนนให้กับโพสต์ใหม่ |
|  |  | should update an existing rating | ทดสอบการอัปเดตคะแนนที่มีอยู่แล้วสำหรับผู้ใช้เดียวกัน |
|  |  | should log an error if updateData throws | ทดสอบกรณีที่เกิดข้อผิดพลาดในการอัปเดตข้อมูลโพสต์แล้วบันทึกข้อผิดพลาดใน log |
|  |  | should return early if no matching posts | ทดสอบกรณีที่ไม่พบโพสต์ที่ตรงกับเงื่อนไขการให้คะแนนและไม่ทำการอัปเดต |
| **PostService** | **UPDATE** | **updateDataInPost** | การทดสอบฟังก์ชัน `updateDataInPost` ที่ใช้ในการอัปเดตข้อมูลโพสต์ |
|  |  | should update post description | ทดสอบการอัปเดตคำบรรยายของโพสต์ |
|  |  | should delete image when deleteImg is true | ทดสอบการลบรูปภาพเมื่อมีการตั้งค่า `deleteImg` เป็น `true` |
|  |  | should upload new image and update postImgUrl | ทดสอบการอัปโหลดรูปภาพใหม่และอัปเดต URL รูปภาพในโพสต์ |
|  |  | should do nothing if no posts found | ทดสอบกรณีที่ไม่พบโพสต์ที่ตรงกับชื่อที่ต้องการอัปเดต |
| **PostService** | **DELETE** | **removePostsByAction** | การทดสอบการลบโพสต์โดยใช้เงื่อนไขต่างๆ เช่น ชื่อโพสต์หรือคะแนน |
|  |  | should remove posts by title and delete images if present | ทดสอบการลบโพสต์ตามชื่อและลบรูปภาพถ้ามี |
|  |  | should remove posts by rating | ทดสอบการลบโพสต์ตามคะแนน |
|  |  | should not remove anything if rating is not a number | ทดสอบกรณีที่คะแนนไม่เป็นตัวเลขจะไม่ลบโพสต์ |
|  |  | should not proceed if repo is empty | ทดสอบกรณีที่ไม่มีโพสต์ใน repository จะไม่ทำการลบโพสต์ |
|  |  | should handle and log errors | ทดสอบกรณีที่เกิดข้อผิดพลาดในการลบโพสต์แล้วบันทึกข้อผิดพลาดใน log |
|  |  | should not remove anything if action is unknown | ทดสอบกรณีที่การกระทำไม่ตรงกับการระบุจะไม่ทำการลบโพสต์ |
| **PostService** | **DELETE** | **removeFirstPostWithImage** | การทดสอบฟังก์ชันการลบโพสต์ตัวแรกที่มีหรือไม่มีรูปภาพ |
|  |  | should not attempt to remove image if first post is null | ทดสอบกรณีที่โพสต์ตัวแรกเป็น null จะไม่พยายามลบรูปภาพ |
|  |  | should remove first post and delete image if present | ทดสอบการลบโพสต์ตัวแรกและลบรูปภาพถ้ามี |
|  |  | should do nothing if post list is empty | ทดสอบกรณีที่รายการโพสต์ว่างจะไม่ทำการลบ |
|  |  | should handle errors and log them | ทดสอบกรณีที่เกิดข้อผิดพลาดในการลบโพสต์และบันทึกข้อผิดพลาดใน log |
| **PostService** | **DELETE** | **removeLastPostWithImage** | การทดสอบฟังก์ชันการลบโพสต์ตัวสุดท้ายที่มีหรือไม่มีรูปภาพ |
|  |  | should remove last post with image | ทดสอบการลบโพสต์ตัวสุดท้ายที่มีรูปภาพ |
|  |  | should remove last post without image | ทดสอบการลบโพสต์ตัวสุดท้ายที่ไม่มีรูปภาพ |
|  |  | should do nothing if list is empty | ทดสอบกรณีที่รายการโพสต์ว่างจะไม่ทำการลบ |
|  |  | should catch and log error | ทดสอบกรณีที่เกิดข้อผิดพลาดในการลบโพสต์สุดท้ายแล้วบันทึกข้อผิดพลาดใน log |